...
import { ResourceAccessLevel, VolumeFormat } from "../../src/enums";

const TEST_GUEST_NAME = "ci-test-guest-volume-v2";
...
describe("云主机磁盘卷管理", () => {
  ...
  beforeAll(async () => {
    ...
    if (guests.data) {
      const existing = guests.data.records.find(g => g.name === TEST_GUEST_NAME || g.name === "ci-test-guest-volume");
      if (existing) {
  ...
  it("addVolume 添加数据磁盘", async () => {
    if (!guestID) return;
    const result = await connector.addVolume(guestID, {
      tag: "data-disk-test",
