...
- 「修改云主机套餐」接口（`tryReplaceGuestConfig`）改为按统一任务约定返回任务 ID：原先该接口走「同步命令空响应」的特殊路径，调用方拿不到任何句柄、无法跟踪整体执行进度与最终结果，只能在结果未知的情况下提示「已加入队列」。配合节点端聚合任务改造，连接器现在按其余写命令的统一格式解析响应、返回任务 ID，调用方可基于该 ID 通过 `getTask` 轮询进度或 `waitTask` 阻塞至完成。这是一项面向调用方的破坏性变更：必须配合节点端同窗口升级，否则在旧节点上调用会因响应中缺少任务 ID 解析失败。

### 新增

- 提供「修改云主机套餐」便捷包装 `replaceGuestConfig`，封装「提交命令 → 阻塞等待聚合任务完成」的常见用法，默认等待时间放宽到十分钟以适配磁盘扩容等耗时步骤；调用方可按业务实际预估覆盖该值。失败时直接透传节点上报的失败原因。
...
- 任务类型常量补齐：调用方在监听任务列表时可识别套餐替换、CPU 优先级 / 磁盘 QoS / 网络 QoS / 流量限额调整、流量计数重置与临时扩量、安全策略调整、内外部网卡 MAC 修改、高可用接管等此前会被解析为原始字符串的任务类型，便于在自定义面板中按类型分类展示与统计
