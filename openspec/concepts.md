...
1. **控制节点 (Control Node)**：集群的大脑，运行 `ControlAPI` 和 `TaskManager`。负责全局状态管理、认证、调度和任务协调。通常不直接运行云主机业务。
2. **资源节点 (Resource Node)**：运行 `ResourceAPI`，负责直接管理本地虚拟化资源（KVM/Qemu）、存储和网络。云主机实际运行在这些节点上。

### 核心组件

- **ControlAPI**：对外提供的统一入口（通常在 5851 端口）。连接器主要与此组件通信。
- **ResourceAPI**：控制节点通过内部网络调用资源节点上的此组件，执行具体的底层操作。
- **TaskManager**：任务管理器，负责处理耗时的异步操作。它维护任务队列、进度状态，并确保操作的原子性和一致性。

### 同步命令 vs 异步命令

在连接器中，命令被分为两类：

1. **同步命令 (Synchronous Commands)**：
   - **特点**：操作几乎立即完成，或由控制节点直接处理。
   - **表现**：API 直接返回操作结果（`BackendResult<T>`）。
   - **示例**：查询云主机列表、修改安全策略、获取监控数据。
2. **异步命令 (Asynchronous Commands)**：
   - **特点**：操作耗时较长（如创建云主机、克隆磁盘、快照恢复），需要跨节点协同或大量 I/O。
   - **表现**：`tryXxx` 方法返回一个 `taskID`。必须使用 `waitTask` 轮询该 ID 直到任务完成。
   - **便捷封装**：连接器为常用的异步操作提供了同名便捷方法（如 `createGuest`），内部自动执行 `try` + `wait` 逻辑，表现上类似于同步调用，但带有超时控制。
