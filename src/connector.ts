...
export class TaiyiConnector {
  ...
  private validateTokens(tokens: AllocatedTokens): string {
    ...
    const mask = (str: string) =>
      str ? `${str.slice(0, 4)}...${str.slice(-4)}` : "null";
    ...
    if (now - ToleranceDuration > accessExpired.getTime()) {
      return `用户${
        tokens.user
      }访问令牌过期。过期时间: ${accessExpired.toLocaleString()}, 客户端当前时间: ${new Date(
        now
      ).toLocaleString()} (允许偏差: 10分钟)。`;
    }
    const refreshExpired = new Date(tokens.refresh_expired_at);
    if (now - ToleranceDuration > refreshExpired.getTime()) {
      return `用户${
        tokens.user
      }刷新令牌过期。过期时间: ${refreshExpired.toLocaleString()}, 客户端当前时间: ${new Date(
        now
      ).toLocaleString()} (允许偏差: 10分钟), 刷新令牌: ${mask(
   ...
   */
  public async replaceGuestConfig(
    params: import('./request-params').ControlReplaceGuestConfigParams,
    timeoutSeconds: number = 600
  ): Promise<BackendResult<TaskData>> {
    const taskResult = await this.tryReplaceGuestConfig(params);
    if (taskResult.error) {
      return { error: taskResult.error };
    }
    const taskData = await this.waitTask(taskResult.data!, timeoutSeconds);
